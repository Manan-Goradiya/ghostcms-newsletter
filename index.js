//This file is just for reference, it is not passed in the code from here. you can add this file in location /var/lib/ghost/content/adapters/storage/ghost-storage-adapter-s3/index.js whenever things break from s3 adapter point of view
//
...
class Store extends _ghostStorageBase2.default {
  constructor() {
    ...
    this.host = process.env.GHOST_STORAGE_ADAPTER_S3_ASSET_HOST || assetHost || 
  ...
  save(image, targetDir) {
    ...
    return new Promise(function (resolve, reject) {
      // Debug: Print current AWS credentials
      console.log('Current AWS Credentials:', {
        accessKeyId: _this3.accessKeyId,
        secretAccessKey: _this3.secretAccessKey,
        region: _this3.region,
        bucket: _this3.bucket,
        endpoint: _this3.endpoint || 'default'
      });

      Promise.all([_this3.getUniqueFileName(image, directory), readFileAsync(image.path)])
        .then(function (_ref) {
          ...
          // Debug: Print the exact S3 client configuration
          console.log('S3 Client Config:', {
            credentials: _this3.s3().config.credentials,
            region: _this3.s3().config.region,
            endpoint: _this3.s3().config.endpoint
          });

          _this3.s3().putObject(config, function (err, data) {
            if (err) {
              // Debug: Print full error with credentials
              console.error('S3 Upload Failed:', {
                error: err,
                credentialsUsed: {
                  accessKeyId: _this3.accessKeyId,
                  secretAccessKey: _this3.secretAccessKey,
                  sessionToken: _this3.s3().config.credentials?.sessionToken
                },
                requestDetails: config
              });
              reject(err);
            } else {
              resolve(`${_this3.host}/${fileName}`);
            }
          });
        })
        .catch(function (err) {
          // Debug: Print file read error with credentials
